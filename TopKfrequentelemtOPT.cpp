...
int main(){
    ...
    for(int n: nums){
        ...
        reps[n] = 1 + reps[n];   // count of every element
    }

    for(const auto var:reps){
        freq[var.second].push_back(var.first);    // pass how many times the elemet occures at index and the element itself as its value 
    }
